...
from segmentator.nnunetv2_custom.preprocessing.cropping.cropping import (
    crop_to_colon,
    crop_to_label,
)
...
class DefaultPreprocessor(object):
    ...
    def run_case_npy(
        ...
        if self.crop_mode == "colon":
            ...
            data, seg, bbox = crop_to_label(data, seg[0], spacing)
            seg = seg[None, ...]
        else:
            data, seg, bbox = (
                data,
        ...
        if seg is not None and np.max(seg) > 127:
    ...
    def run_case(
        ...
        case_id = f"{int(Path(image_files[0]).name.replace('_0000.nii.gz', ''))}.nii.gz"
    ...
    def _normalize(
        ...
        for c in range(data.shape[0]):
            ...
            normalizer_class = recursive_find_python_class(
                join(
                    segmentator.nnunetv2_custom.__path__[0],
                    "preprocessing",
                    "normalization",
                ),
                scheme,
                "segmentator.nnunetv2_custom.preprocessing.normalization",
            )

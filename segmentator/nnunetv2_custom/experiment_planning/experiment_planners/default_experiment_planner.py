...
from segmentator.nnunetv2_custom.configuration import ANISO_THRESHOLD
from segmentator.nnunetv2_custom.experiment_planning.experiment_planners.network_topology import (
    get_pool_and_conv_props,
)
from segmentator.nnunetv2_custom.imageio.reader_writer_registry import (
    determine_reader_writer_from_dataset_json,
)
from segmentator.nnunetv2_custom.paths import nnUNet_preprocessed, nnUNet_raw
from segmentator.nnunetv2_custom.preprocessing.normalization.map_channel_name_to_normalization import (
    get_normalization_scheme,
)
from segmentator.nnunetv2_custom.preprocessing.resampling.default_resampling import (
    compute_new_shape,
    resample_data_or_seg_to_shape,
)
from segmentator.nnunetv2_custom.utilities.dataset_name_id_conversion import (
...
from segmentator.nnunetv2_custom.utilities.json_export import (
    recursive_fix_for_json_export,
)
from segmentator.nnunetv2_custom.utilities.utils import (
...
class ExperimentPlanner(object):
    ...
    def plan_experiment(self):
        """
        Experiment planning with FORCED 3d_lowres for cascaded training.
        """
        ...
        if new_median_shape_transposed[0] != 1:
            ...
            # ==================================================
            # FORCED 3D LOWRES (KEY CHANGE)
            # ==================================================

            # --- MANUAL LOW-RES SPACING ---
            # Change this if you want coarser / finer low-res
            lowres_spacing = np.array([2.0, 2.0, 2.0], dtype=np.float32)

            # Compute corresponding low-res shape
            lowres_shape = np.round(
                plan_3d_fullres["spacing"]
                / lowres_spacing
                * new_median_shape_transposed
            ).astype(int)

            median_num_voxels_lowres = np.prod(lowres_shape, dtype=np.float64)

            plan_3d_lowres = self.get_plans_for_configuration(
                lowres_spacing,
                tuple(lowres_shape),
                self.generate_data_identifier("3d_lowres"),
                float(median_num_voxels_lowres * self.dataset_json["numTraining"]),
                _tmp,
            )

            # Dice handling (as in nnU-Net)

...
class DatasetFingerprintExtractor(object):
    ...
    def analyze_case(
        ...
        if crop_mode == "colon":
            data_cropped, seg_cropped, bbox = crop_to_colon(
                images, segmentation, case_id, spacing
            )
        elif crop_mode == "gt":
            data_cropped, seg_cropped, bbox = crop_to_label(
                images, segmentation[0], spacing
            )
            seg_cropped = seg_cropped[None, ...]
        else:

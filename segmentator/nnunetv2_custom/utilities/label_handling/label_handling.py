...
from segmentator.nnunetv2_custom.utilities.helpers import softmax_helper_dim0
...
def get_labelmanager_class_from_plans(plans: dict) -> Type[LabelManager]:
    if "label_manager" not in plans.keys():
        ...
        labelmanager_class = recursive_find_python_class(
            join(
                segmentator.nnunetv2_custom.__path__[0], "utilities", "label_handling"
            ),
            plans["label_manager"],
            current_module="segmentator.nnunetv2_custom.utilities.label_handling",
        )

...
class DatasetFingerprintExtractor(object):
    def __init__(self, dataset_name_or_id: Union[str, int], num_processes: int = 8, verbose: bool = False, crop_mode: str="none"):
        """
    ...
    def analyze_case(image_files: List[str], segmentation_file: str, reader_writer_class: Type[BaseReaderWriter],
                     num_samples: int = 10000, crop_mode: int = "none"):
        ...
        if crop_mode=="colon":
            ...
            data_cropped, seg_cropped, bbox = crop_to_label(images, segmentation[0],spacing)
            seg_cropped = seg_cropped[None,...]
        else: 
            data_cropped, seg_cropped , bbox = images, segmentation, [[0, images.shape[1] - 1],[0, images.shape[2] - 1],[0, images.shape[3] - 1]]
                ...
                for k in self.dataset.keys():
                    r.append(p.starmap_async(DatasetFingerprintExtractor.analyze_case,
                                             ((self.dataset[k]['images'], self.dataset[k]['label'], reader_writer_class,
                                               num_foreground_samples_per_case,self.crop_mode),)))

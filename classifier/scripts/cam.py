...
class ScoreCAM3D(BaseCAM3D):
    ...
    def compute(self, x: torch.Tensor, class_index: int) -> torch.Tensor:
        ...
        for c in range(num_channels):
            ...
            weight = (torch.sigmoid(logits)[:, 0].mean() if logits.shape[1] == 1 else torch.softmax(logits, dim=1)[:, class_index].mean())
...
def run_cam_pipeline(method: str, normalization_mode: str, cfg: Dict, device: torch.device, logger: logging.Logger) -> None:
    ...
    output_dir = path_root / cam_cfg["output_dir"] / cam_cfg["method"] / cam_cfg["normalization_mode"] / chkpt_folder.name
    ...
    extractor: BaseCAM3D = GradCAM3D(model, target_layer, device) if method.lower() == "gradcam" else ScoreCAM3D(model, target_layer, device)
    ...
    for uid, cam_list in tqdm(all_cams.items(), desc=f"Saving {method} outputs"):
        ...
        for cam, vol, lbl, patch_idx, pred, prob in cam_list:
            ...
            overlay_and_save_slices(vol, cam, cmin, cmax, lbl, out_png, alpha=alpha, num_slices=cam_cfg["num_slices"])

...
from segmentator.nnunetv2_custom.imageio.natural_image_reader_writer import (
    NaturalImage2DIO,
)
from segmentator.nnunetv2_custom.imageio.nibabel_reader_writer import (
    NibabelIO,
    NibabelIOWithReorient,
)
from segmentator.nnunetv2_custom.imageio.simpleitk_reader_writer import SimpleITKIO
from segmentator.nnunetv2_custom.imageio.tif_reader_writer import Tiff3DIO
...
def recursive_find_reader_writer_by_name(rw_class_name: str) -> Type[BaseReaderWriter]:
    ret = recursive_find_python_class(
        join(segmentator.nnunetv2_custom.__path__[0], "imageio"),
        rw_class_name,
        "segmentator.nnunetv2_custom.imageio",
    )
    if ret is None:
        raise RuntimeError(
            "Unable to find reader writer class '%s'. Please make sure this class is located in the "
            "segmentator.nnunetv2_custom.imageio module." % rw_class_name
        )
    else:
        return ret

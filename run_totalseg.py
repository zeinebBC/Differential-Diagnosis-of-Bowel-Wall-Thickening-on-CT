...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Batch TotalSegmentator colon inference (Python API)"
    )
    parser.add_argument(
        "--input", required=True, help="Folder with input NIfTI volumes"
    )
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing masks"
    )
    parser.add_argument(
        "--task", type=str, default="total", help="Name of the segmentation task"
    )
    parser.add_argument(
        "--roi_subset",
        type=str,
        nargs="+",
        default=None,
        help="specify the region(s) of interest",
    )
